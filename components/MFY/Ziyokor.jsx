...
const getTopics = async () => {
    ...
    try {
        ...
        const filteredTopics = topiclar.filter(topic => {
            ...
            return createdAt > filterDate && topic.MFY === "ZIYOKOR MFY";
        });
    ...
    useEffect(() => {
        const fetchTopics = async () => {
            try {
                ...
                // Filter topics by MFY equal to "CHORTOQ MFY"
                const filteredTopicsByMFY = newFilteredTopics.filter(topic => topic.MFY === "ZIYOKOR MFY");

                setTopicCount(filteredTopicsByMFY.length);

                if (filteredTopicsByMFY.length > 0) {
                    const latestTopicId = filteredTopicsByMFY[filteredTopicsByMFY.length - 1]._id;
